...
[pxxdat,fdat] = periodogram(dataVec, [], [], sampFreq); %total periodogram
figure;
plot(fdat,pxxdat);
hold on;
[pxxsig,fsig] = periodogram(sigVec, [], [], sampFreq); %signal only periodogram
plot(fsig,pxxsig);
xlabel('Frequency(Hz)');
ylabel('Amplitude');
title('periodgram');
...
figure;
spectrogram(dataVec,256,200,512,sampFreq,'yaxis');